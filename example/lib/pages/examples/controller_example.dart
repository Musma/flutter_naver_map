...
import '../../util/snapshot_image_stub.dart' if (dart.library.io) '../../util/snapshot_image_io.dart';
...
class NaverMapControllerExample extends StatefulWidget {
  static const ExamplePageData pageData = ExamplePageData(
      title: "기타 컨트롤러 기능", description: "컨트롤러 기능을 살펴봐요", icon: Icons.sports_esports_rounded, route: "/controller_etc");
  ...
  @override
  State<NaverMapControllerExample> createState() => _NaverMapControllerExampleState();
}
...
class _NaverMapControllerExampleState extends State<NaverMapControllerExample> {
  ...
  void onCameraChange() async {
    ...
    _lastTappedScreenPosition = await _mapController.latLngToScreenLocation(_nowCameraPosition!.target);
  ...
  Widget _nowCameraPositionWidget() {
    return Container(
        ...
        child: Column(children: [
          const InnerSimpleTitle(title: "현재 카메라 상태", description: ".nowCameraPosition", direction: Axis.horizontal),
          const SizedBox(height: 8),
          Row(children: [
            _cameraPositionValueElementWidget("위도", _nowCameraPosition?.target.latitude),
            _cameraPositionValueElementWidget("경도", _nowCameraPosition?.target.longitude),
            _cameraPositionValueElementWidget("방위각", _nowCameraPosition?.bearing),
  ...
  Widget _cameraPositionValueElementWidget(String title, Object? value) {
    return Expanded(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(value.toString(), overflow: TextOverflow.ellipsis, style: getTextTheme(context).bodySmall),
      ]),
    ));
  }
    ...
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
            ...
            Text("${_nowMeterPerDp}m", style: getTextTheme(context).titleMedium),
          ]),
        ]));
  }
  ...
  Widget _contentsRegionWidget() {
    return Container(
        padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 8),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const InnerSimpleTitle(title: "지도 위젯에 보여지는 범위", description: ".getContentBounds() | .getContentRegion()"),
          const SizedBox(height: 4),
          Text(
              "남서: ${_regionBounds?.southWest.toShortString()}, "
              "북동: ${_regionBounds?.northEast.toShortString()}",
              style: getTextTheme(context).bodyMedium?.copyWith(fontWeight: FontWeight.w700)),
        ]));
  }
  ...
  Widget _switchLatLngToScreenLocationWidget() {
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const InnerSimpleTitle(
              title: "위치 좌표 ↔️ 화면 좌표",
              description: ".screenLocationToLatLng(NPoint)\n.latLngToScreenLocation(NLatLng)"),
          const SizedBox(height: 4),
          Text(
  ...
  Widget _actionButtonSections() {
    return Padding(
      ...
      child: HalfActionButtonGrid(buttons: [
        ...
        HalfActionButton(action: _takeSnapshot, icon: Icons.camera_alt, title: "지도 캡쳐하기", description: ".takeSnapshot"),
      ]),
    );
  }
            ...
            return Center(
              ...
              child: Container(
                  ...
                  child:
                    ...
                    Text("지도 캡쳐하기", style: getTextTheme(context).titleMedium),
                    Container(
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      Expanded(
          child: ListView(padding: EdgeInsets.only(bottom: MediaQuery.paddingOf(context).bottom), children: [
  ...
  void initState() {
    super.initState();
    onCameraChange();
    onCameraChangeStreamSubscription = widget.onCameraChangeStream.listen((_) => onCameraChange());
  }
